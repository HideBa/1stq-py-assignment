...
Circle circumcircle(Triangle t)
{
  ...
  Point center = {
      (
          (t.a.x * t.a.x + t.a.y * t.a.y) * (t.b.y - t.c.y) + (t.b.x * t.b.x + t.b.y * t.b.y) * (t.c.y - t.a.y) + (t.c.x * t.c.x + t.c.y * t.c.y) * (t.a.y - t.b.y)) /
          D,
      (
          (t.a.x * t.a.x + t.a.y * t.a.y) * (t.c.x - t.b.x) + (t.b.x * t.b.x + t.b.y * t.b.y) * (t.a.x - t.c.x) + (t.c.x * t.c.x + t.c.y * t.c.y) * (t.b.x - t.a.x)) /
          D};
...
void printCircleWKT(const Circle &c, std::ostream &stream, int num_segments = 100)
{
  ...
  wkt += std::to_string(c.center.x + c.radius) + " " + std::to_string(c.center.y); // first point
  for (int i = 1; i < num_segments; ++i)
  {
    ...
    wkt += ", ";
    wkt += std::to_string(c.center.x + dx) + " " + std::to_string(c.center.y + dy);
  }
  wkt += ", ";
  wkt += std::to_string(c.center.x + c.radius) + " " + std::to_string(c.center.y); // last
  wkt += "))\n";
  // stream << wkt;
}
...
{
  std::string wkt = "POLYGON ((";
  wkt += std::to_string(t.a.x) + " " + std::to_string(t.a.y) + ", ";
  wkt += std::to_string(t.b.x) + " " + std::to_string(t.b.y) + ", ";
  wkt += std::to_string(t.c.x) + " " + std::to_string(t.c.y) + ", ";
  wkt += std::to_string(t.a.x) + " " + std::to_string(t.a.y);
  wkt += "))\n";
  // stream << "POLYGON (("
  //        << t.a.x << " " << t.a.y << ", "
...
void printPointWKT(const Point &p, std::ostream &stream)
{
  ...
  wkt += std::to_string(p.x) + " " + std::to_string(p.y);
  wkt += ")\n";
  // stream << "POINT ("
  //        << p.x << " " << p.y
  //        << ")" << std::endl;
